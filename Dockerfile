...
RUN openssl pkcs12 -export -inkey eventstore.pem -in eventstore.csr -out eventstore.p12 -password pass:changeit

RUN cp eventstore.csr /usr/local/share/ca-certificates/eventstore.crt

RUN update-ca-certificates

RUN cert-sync eventstore.csr

RUN apt-get -y remove mono-devel openssl \
